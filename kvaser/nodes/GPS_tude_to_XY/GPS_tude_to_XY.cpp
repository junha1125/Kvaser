...
{	
	...
	{ 	
		...
		if(temp_dix > 6.0 || temp_diy > 2.10 || temp_diy < 1.5 ) //Limit car dimensions
		{
			change = 0;
			continue;
		...
		double center_x = Center_x(temp_x,temp_dix); //Car center
		double center_y = Center_y(temp_y,temp_dix);

		if(center_y > 8.5 || center_y < -8.5 || center_x > 0 || center_x <-10  ) //Lidar cropping
		{	
			change = 0;		
			continue;
		...
		if(center_x >-1.55 && center_x < 0) //Lidar area
		{
			change = 0;
			continue;
		}

		center_y = -center_y;  //reverse the lidar coordinate to fit uc_coordinate 
		...
		double theta = atan2(temp_uc_Y2-temp_uc_Y1, temp_uc_X2-temp_uc_X1); //Adding relative coordinate
		...
		{	temp_uc_X5 = temp_uc_X2 + center_x*cos(theta) + Ydistance * sin(theta);
			...
			//call_socket6(center_x,center_y);
			//call_socket5(temp_uc_X3,temp_uc_Y3);
			//call_socket4(temp_uc_X4,temp_uc_Y4);
		...
		{	temp_uc_X3 = temp_uc_X2 + center_x*cos(theta) - Ydistance * sin(theta);
			...
			//call_socket4(center_x,center_y);
			//call_socket5(temp_uc_X3,temp_uc_Y3);
			//call_socket6(temp_uc_X4,temp_uc_Y4);
		...
		{	temp_uc_X4 = temp_uc_X2 + center_x*cos(theta);	
			...
			//call_socket5(center_x,center_y);
			//call_socket4(temp_uc_X3,temp_uc_Y3);
			//call_socket6(temp_uc_X4,temp_uc_Y4);


		} 

		std::cout << "car_uc_x: " << center_x <<std::endl; //Gaining UC/win coordinate
		std::cout << "car_uc_y: " << center_y <<std::endl;
		std::cout << '\n';
...
void chatterCallback(const sensor_msgs::NavSatFix::ConstPtr& msg)
{
	...
	else
	{
		...
		if(gpsXYdata.Is_change == 1){
			...
			//temp_uc_x = gpsXYdata.uc_x;
			//temp_uc_y = gpsXYdata.uc_y;
...
int main (int argc, char *argv[]){
  ...
  ros::init(argc, argv, "GPS_tude_to_XY");					// Node이름이 GPS_tude_to_XY이다. 
  ros::NodeHandle n;
